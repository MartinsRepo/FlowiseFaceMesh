...
def convert_protobuf_to_dict(pb_msg):
    """
    Converts a protobuf message (of type facemesh_pb2.FD) to a python dictionary.

    Args:
        pb_msg: The protobuf message to convert.

    Returns:
        A python dictionary representing the data in the protobuf message.
    """
    frame_data = {}
    frame_data['message'] = pb_msg.filename  # Get filename
...
def ecal_subscriber():
    ...
    # define subscribing message
    sub = ProtoSubscriber("FaceCoords", facemesh_pb2.FD)
    
    def callback(topic_name, msg, time):
        try:
            # Convert the protobuf message to a python dictionary
            frame_data = convert_protobuf_to_dict(msg)  # Use the conversion function
            # print(f"Received frame data: {frame_data}") # For debugging
            json_data = json.dumps(frame_data, indent=2)
            frame_queue.put(json_data, block=False)
        except frame_queue.full:
            # Queue is full, dropping this frame
            print("Queue full. Dropping frame.")
        except Exception as e:
            print(f"Error processing message: {e}")
            frame_queue.put(json.dumps({"error": str(e)}))  # Put error in queue as json
...
def ecalrunner():
    if ecal_core.ok():
        ...
        json_data = json.dumps(frame, indent=2)
        #print(json_data)
        
        response = requests.post(
            FLOWISE_API_URL,
            json={"question": json_data}
        )
        
        return_queue.put(response)
        
    ecalrunner = Timer(0.5, json2flowise)
    ecalrunner.start()
...
def json2flowise():
    ...
    flowisethread = Timer(0.5, json2flowise)
    flowisethread.start()
...
if __name__ == "__main__":
    ...
    # start ecal subscriber as timer thread period 500ms
    ecalthread = Timer(0.5,  ecalrunner).start()

    flowisethread = Timer(0.5, json2flowise).start()

    # Keep the main thread alive to allow the timer threads to run
    try:
        while ecal_core.ok():
            time.sleep(0.1)
